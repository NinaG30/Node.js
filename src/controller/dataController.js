...
// une donnée du tableau 'data' présent dans data.json
...
// Exporte la fonction qui permet de récupérer une donnée via son nom du tableau 'data' dans data.json
exports.getDataByName = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      const jsonData = JSON.parse(data);
      const dataByName = jsonData.data.find(
...
//Exporte une fonction qui permet d'ajouter une donnée au tableau 'data' dans data.json avec id dynamique
exports.postData = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      let object = existingData.data.findLast((obj) => obj.id)// cherche dernier objet;      
      let dynamicId = {"id": object.id+1, "name": request.body.name}           
      existingData.data.push(dynamicId);     
      fs.writeFile(
        "./src/model/data.json",
...
// Exporte une fonction permettant de modifier une donnée du tableau 'data' via son id
exports.updateData = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      const existingData = JSON.parse(data);
      const dataById = existingData.data.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
...
// Exporte une fonction qui supprime une donnée du tableau 'data' dans le fichier data.json
