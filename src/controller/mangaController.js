...
// une donnée du tableau 'manga' présent dans data.json
exports.getDataById = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      const dataById = jsonData.manga.find(
...
// Exporte la fonction qui permet de récupérer une donnée via son nom du tableau 'manga' dans data.json
exports.getDataByName = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      const jsonData = JSON.parse(data);
      const dataByName = jsonData.manga.find(
...
//Exporte une fonction qui permet d'ajouter une donnée au tableau 'manga' dans data.json avec id dynamique
exports.postData = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      let object = existingData.manga.findLast((obj) => obj.id)// cherche dernier objet;      
      let dynamicId = {"id": object.id+1, "name": request.body.name}           
      existingData.manga.push(dynamicId); 
      fs.writeFile(
        "./src/model/data.json",
...
// Exporte une fonction permettant de modifier une donnée du tableau 'manga' via son id
exports.updateData = (request, response) => {
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      const existingData = JSON.parse(data);
      const dataById = existingData.manga.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
...
// Exporte une fonction qui supprime une donnée du tableau 'manga' dans le fichier data.json
  ...
  fs.readFile("./src/model/data.json", (err, data) => {
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.manga = existingData.manga.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
        fs.writeFile(
          "./src/model/data.json",
